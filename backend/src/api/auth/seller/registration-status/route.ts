 ...
 *
 * This endpoint is placed under /auth/ instead of /vendor/ to avoid
 * the MercurJS b2c-core plugin's vendor authentication middleware.
 *
 * It verifies the bearer token manually to support pending users
 * who have an auth_identity but no seller_id yet.
 */
export async function GET(req: MedusaRequest, res: MedusaResponse) {
  console.log("[GET /auth/seller/registration-status] Handler reached - AUTHENTICATE=false bypasses auth middleware")

  try {
    // Extract bearer token from Authorization header
    const authHeader = req.headers.authorization
    console.log(`[GET /auth/seller/registration-status] Auth header present: ${!!authHeader}`)

    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      console.log("[GET /auth/seller/registration-status] No Bearer token found")
    ...
    console.log(`[GET /auth/seller/registration-status] Token length: ${token.length}`)
    ...
    try {
      ...
      if (tokenParts.length === 3) {
        ...
        console.log(`[GET /auth/seller/registration-status] JWT payload fields: ${Object.keys(payload).join(', ')}`)
        ...
        console.log(`[GET /auth/seller/registration-status] Decoded - authIdentityId: ${authIdentityId ? 'found' : 'null'}, sellerId: ${sellerId ? 'found' : 'null'}`)
      }
    } catch (decodeError: any) {
      console.log(`[GET /auth/seller/registration-status] JWT decode error: ${decodeError.message}`)
    }
    ...
    if (!authIdentityId && (req as any).auth_context?.auth_identity_id) {
      ...
      console.log(`[GET /auth/seller/registration-status] Got auth from context - authIdentityId: ${!!authIdentityId}, sellerId: ${!!sellerId}`)
    }
    ...
    if (!authIdentities || authIdentities.length === 0) {
      console.log(`[GET /auth/seller/registration-status] Auth identity not found: ${authIdentityId}`)
      return res.status(401).json({
        status: "unauthenticated",
    ...
    console.log(`[GET /auth/seller/registration-status] Auth identity found, app_metadata:`, JSON.stringify(authIdentity.app_metadata))
    ...
    if (appMetadata?.seller_id) {
      console.log(`[GET /auth/seller/registration-status] Found seller_id in app_metadata: ${appMetadata.seller_id}`)
      return res.json({
        status: "approved",
    ...
    console.log(`[GET /auth/seller/registration-status] No seller_id in app_metadata, checking pending requests...`)
    ...
    console.log(`[GET /auth/seller/registration-status] Found ${userRequests.length} requests for this user`)
    ...
    console.log(`[GET /auth/seller/registration-status] Latest request status: ${latestRequest.status}, id: ${latestRequest.id}`)
    ...
    switch (latestRequest.status) {
      ...
      case "accepted": {
        ...
        console.log(`[GET /auth/seller/registration-status] Request accepted but no seller_id in app_metadata, looking up seller...`)
        ...
        if (memberEmail) {
          try {
            ...
            if (existingSeller) {
              console.log(`[GET /auth/seller/registration-status] Found seller ${existingSeller.id}, updating auth_identity...`)
              ...
              console.log(`[GET /auth/seller/registration-status] Auth identity updated with seller_id`)
            ...
            console.error(`[GET /auth/seller/registration-status] Error looking up seller:`, lookupError.message)
          }
        }
    ...
    console.error("[GET /auth/seller/registration-status] Error:", errorMessage)
