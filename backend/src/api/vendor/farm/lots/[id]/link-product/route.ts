...
interface AgricultureServiceType {
  updateAvailabilityWindows: (data: Record<string, unknown>) => Promise<{ id: string }>
...
export async function POST(
  ...
  try {
    ...
    if (sync_inventory && variant_id) {
      // This would sync Medusa inventory with lot quantity
      // Implementation depends on your inventory setup
      // For now, we'll just log it
      console.log(`Would sync inventory for variant ${variant_id} with lot ${lotId}`)
    }
...
export async function GET(
  ...
  try {
    ...
    const linkedProducts = (windows || []).map((window: Record<string, unknown>) => {
      const product = products.find((p: Record<string, unknown>) => p.id === window.product_id)
      return {
        ...window,
        product,
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (!availability_window_id) {
      return res.status(400).json({ message: "availability_window_id is required" })
    }
