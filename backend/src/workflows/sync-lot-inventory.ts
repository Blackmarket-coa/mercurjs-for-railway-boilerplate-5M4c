...
const getLotQuantityStep = createStep(
  ...
  async (input: SyncLotInventoryInput, { container }) => {
    ...
    const lot = lots[0] as { id: string; quantity_available: number; quantity_reserved: number; unit: string }
...
const updateInventoryStep = createStep(
  ...
  async (
    ...
    const inventoryService = container.resolve("inventory") as {
      updateInventoryLevels: (data: { inventory_item_id: string; location_id: string; stocked_quantity: number; reserved_quantity: number }[]) => Promise<unknown>
    }
    const query = container.resolve("query")
    ...
    if (!variantInventory || variantInventory.length === 0) {
      ...
      console.log(`No inventory item found for variant ${input.product_variant_id}`)
      return new StepResponse({ synced: false, reason: "no_inventory_item" })
    }

    const inventoryItemId = (variantInventory[0] as { inventory_item_id: string }).inventory_item_id
